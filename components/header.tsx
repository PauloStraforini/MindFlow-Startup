...
const Header = () => {
  ...
  return (
    ...
    >
          ...
          {["inicio", "funcionalidades", "beneficios", "depoimentos", "precos", "contato"].map(
            (item) => (
              <button
                ...
                className={`relative py-1 px-2 text-sm font-medium transition-colors duration-300 ${activeSection === item ? "text-pink-600" : "text-gray-600"
                  } hover:text-pink-600`}
              >
                {item.charAt(0).toUpperCase() + item.slice(1)}
                <span
                  className={`absolute bottom-0 left-0 h-0.5 bg-pink-500 transition-all duration-300 rounded-full ${activeSection === item ? "w-full" : "w-0"
                    }`}
                />
              </button>
            )
      ...
      {/* Mobile nav list */}
      <div
        className={`md:hidden absolute w-full bg-white border-b border-pink-100 shadow-lg transition-all duration-500 ease-in-out overflow-hidden transform ${isMenuOpen ? "max-h-screen opacity-100 translate-y-0" : "max-h-0 opacity-0 -translate-y-2"
          }`}
      >
          ...
          {["inicio", "funcionalidades", "beneficios", "depoimentos", "preços", "contato"].map(
            (item) => (
              <button
                key={item}
                onClick={() => scrollToSection(item)}
                className={`text-left py-2 text-sm transition-colors duration-300 ${activeSection === item ? "text-pink-600 font-medium" : "text-gray-600"
                  } hover:text-pink-600`}
              >
                {item.charAt(0).toUpperCase() + item.slice(1)}
              </button>
